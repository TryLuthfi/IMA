package indonesia.asperindo.com.asperindo.konfigurasi;

public class konfigurasi {
    public static final String TAG_JSON_ARRAY = "dataacara";
    public static final String TAG_JSON_ARRAY2 = "dataartikel";
    public static final String TAG_JSON_ARRAY3 = "datauser";
    public static final String TAG_JSON_ARRAY4 = "dataima";

    public static final String id_acara = "id_acara";
    public static final String jamAcara = "jam_acara";
    public static final String tanggalAcara = "tanggal_acara";
    public static final String alamatAcara = "alamat_acara";
    public static final String judulAcara = "judul_acara";
    public static final String Alamatlengkap_acara = "alamat_lengkap_acara";
    public static final String deskripsiAcara = "deskripsi_acara";
    public static final String gambarAcara = "gambar_acara";

    public static final String id_user = "id_user";
    public static final String id_artikel = "id_artikel";
    public static final String judul_artikel = "judul_artikel";
    public static final String gambar_artikel = "gambar_artikel";
    public static final String deskripsi_artikel = "deskripsi_artikel";
    public static final String artikel_created_date = "artikel_created_date";


    public static final String nama = "nama";
    public static final String username = "username";
    public static final String password = "password";
    public static final String no_telp = "no_telp";
    public static final String email_pribadi = "email_pribadi";
    public static final String foto_profile = "foto_profile";
    public static final String bidang_usaha = "bidang_usaha";
    public static final String nama_usaha = "nama_usaha";
    public static final String email_usaha = "email_usaha";
    public static final String logo_usaha = "logo_usaha";
    public static final String chapter = "chapter";
    public static final String longitude = "longitude";
    public static final String latitude = "latitude";
    public static final String register_date = "register_date";

    public static final String id_tentang = "id_tentang";
    public static final String nama_tentang = "nama";
    public static final String nama_lengkap = "nama_lengkap";
    public static final String logo = "logo";
    public static final String deskripsi = "deskripsi";

    public static final String URL_GET_DATAACARA = "http://imaindonesia.000webhostapp.com/dataacara.php";
    public static final String URL_GET_DATAARTIKEL = "http://imaindonesia.000webhostapp.com/dataartikel.php";
    public static final String URL_GET_DATAUSER = "http://imaindonesia.000webhostapp.com/datauser.php";
    public static final String URL_GET_DATAIMA = "http://imaindonesia.000webhostapp.com/dataima.php";
    public static final String URL_GET_KONTAK = "http://imaindonesia.000webhostapp.com/userpreview.php";
}
